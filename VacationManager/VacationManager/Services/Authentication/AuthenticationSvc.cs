...
namespace VacationManager.Services.Authentication
{
    public class AuthenticationSvc : BaseService, IAuthenticationSvc
    {
        ...
        public async Task<ResponseModel<string>> Authenticate(string email, string password)
        {
            ...
            if(user != null)
            {
                if (EncryptingHelper.VerifyPassword(user.Password, password))
                {
                    if (_jwtSettings.SecretKey != null)
                    {
                        ...
                        result.Result = new JwtSecurityTokenHandler().WriteToken(token);
                        return result;
                    }
                }
            }

            result.AddError(Common.Enums.ReasonCodeEnum.UserNotFound);
